...
public class _07_BusiestTimeInMall {
 
    static int findBusiestPeriod(int[][] data) {
        // your code goes here
        int maxPeople = 0, maxTime = 0, curPeople = 0;
        int n = data.length;
        for(int i = 0; i < n; i++) {
          int timestamp = data[i][0];
          curPeople += (data[i][2] == 0) ? -1*data[i][1] : data[i][1];       
...
The Westfield Mall management is trying to figure out what the busiest moment at the mall was last year.
 You’re given data extracted from the mall’s door detectors. Each data point is represented as an integer
  ...
  whether the visitors entered or exited the mall (0 for exit and 1 for entrance), respectively. Here’s 
